...
function Card({ card, onDelete }: { card: KanbanCard; onDelete: (id: string) => void }) {
  ...
  return (
    <div ref={setNodeRef} style={{ transition, transform: CSS.Translate.toString(transform) }}
      ...
      <button onClick={() => onDelete(card.id)} className="opacity-0 group-hover:opacity-100 text-muted/40 hover:text-destructive shrink-0 text-xs">×</button>
    </div>
  );
}
      ...
      <div className="flex-1 px-2 pb-2 space-y-1.5 overflow-auto max-h-[62vh]">
        ...
        {adding ? (
          <div className="bg-background border border-border/60 rounded-lg p-2 space-y-1.5">
            <input autoFocus value={title} onChange={e => setTitle(e.target.value)} onKeyDown={e => { if (e.key === "Enter") submit(); if (e.key === "Escape") setAdding(false); }} placeholder="卡片标题..." className="w-full bg-transparent border-none outline-none text-xs text-foreground placeholder:text-muted/40" />
            <div className="flex gap-1.5"><button onClick={submit} className="px-2.5 py-1 text-[10px] bg-primary/10 text-primary rounded-md">添加</button><button onClick={() => setAdding(false)} className="px-2.5 py-1 text-[10px] text-muted/50">取消</button></div>
          </div>
        ) : (
